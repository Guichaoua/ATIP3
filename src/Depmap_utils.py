...
def load_merge_data( dict_filters, list_mutation, drop_mutation=False, keep_only_mutation=False, expr_keep=[]):
    ...
    df_1 = df_1.merge(meta_grouped_Genetic_features, on='DepMap_ID')
    #df_1.drop_duplicates(inplace=True) (je ne peux pas le faire car list avec mutated_genes)
...
def correl_expression_measure_sensitivity(df, measure_col, expr_col='MTUS1'):
    """
    ...
    Args:
        ...
        expr_col (str): Column name for gene expression (default is 'MTUS1').
    ...
    sub_df = df[[expr_col, measure_col]].dropna().drop_duplicates(subset=[expr_col, measure_col])

    # Compute Pearson correlation
    corr, pval = pearsonr(sub_df[expr_col], sub_df[measure_col])
    return corr, pval


def correlations_expression_measure_all_drugs_sensitivity(df, measure_col, expr_col='MTUS1'):
    """
    ...
    Args:
        ...
        measure_col (str): Column name for drug sensitivity (for gdsc2 : 'AUC' or 'LN_IC50', for prism : 'LFC').
        expr_col (str): Expression column (e.g. 'MTUS1').
    ...
    for drug in df['DRUG_NAME'].unique():
        sub = df[df['DRUG_NAME'] == drug][[expr_col, measure_col, 'PUTATIVE_TARGET', 'PATHWAY_NAME']].dropna()
        sub = sub.drop_duplicates(subset=[expr_col, measure_col])
        sub[measure_col] = sub[measure_col].astype(str).str.replace(',', '.')
        sub[measure_col] = pd.to_numeric(sub[measure_col], errors='coerce')
        sub = sub.dropna()

        if len(sub) >= 3:
            x = sub[expr_col].to_numpy()
            y = sub[measure_col].to_numpy()
            r, p = pearsonr(sub[expr_col], sub[measure_col])
...
def plot_expr_vs_measure_sensitivity(
    ...
    Args:
        ...
        measure_col (str): Column name for drug sensitivity (for gdsc2 : 'AUC' or 'LN_IC50', for prism : 'LFC').
        expr_col (str): Column name for gene expression.
    ...
    sub = df[df['DRUG_NAME'] == drug_name].dropna(subset=[expr_col, measure_col]).drop_duplicates(subset=[expr_col, measure_col])
    ...
    sns.regplot(data=sub, x=expr_col, y=measure_col, scatter_kws={'alpha': 0.6})
    ...
    if 'ModelSubtypeFeatures' in sub.columns:
        sns.scatterplot(
            ...
            x=expr_col, y=measure_col,
            label='TNBC', color='r', alpha=0.6
        )

    x_label = f"{expr_col} (log2(TPM+1))"

    # Annotate the plot
    plt.title(f"{drug_name}\nCorrelation {expr_col} vs {measure_col}\nr = {r:.2f}, p = {p:.1e}")
    plt.xlabel(x_label)
